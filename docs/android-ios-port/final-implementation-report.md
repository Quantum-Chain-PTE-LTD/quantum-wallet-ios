...
This report records what was completed in this session, what was already present in iOS, and what is explicitly deferred to follow-up work. Build/test verification was not run in this session (no iOS toolchain on Windows host); all changes were validated by static reasoning only.
...
The following items are in scope per the original plan but are deferred to a follow-up session due to size and verification risk without an iOS build environment. They are listed with the Android refs and iOS files known to be impacted.

### B4 — Quantum price source router (NEW MODULE)
- Andriod refs: `96e4f19d`, `eef9474b`.
- iOS impact: net-new package under `Core/Providers/PriceSource/` containing `PriceSourceRouter`, `QuantumPriceApi`, `TokenPriceMappingConfig`, `TokenPriceSourceMappingService`; wiring into `Core/Managers/MarketKitWrapper.swift` (or equivalent); persistence via `LocalStorage` for TTL + mapping cache.
- Recommended approach: port verbatim from Android, swap Retrofit → `URLSession` + `async/await`; replace RxJava observers → Combine `Publisher`s with `.debounce(for: 30, scheduler:)`.

### C10 — Settings simplification
- Android ref: `79dd2877`.
- iOS files: `Modules/Settings/MainSettings/MainSettingsView*.swift`, `Modules/Settings/Appearance/*`, `Modules/Settings/AppIcon/*`, `Modules/Settings/About/*`, `Modules/Settings/Support/*`.
- Recommended: remove rows for theme selector, app icon chooser, About, FAQ, Guides, Support; collapse to: Security, Currency, Language, Contacts, Logout (or per Android final list).

### D11 — Remove WalletConnect
- Android refs: `90c385c1`, `b67e81e6`.
- iOS impact: entire `Modules/WalletConnect/` tree (~30+ files), `Core/Managers/Wallet*WalletConnect*.swift`, WC SwiftPM dependency in `project.pbxproj`, deeplink scheme entries in `Info.plist`, `wc.*` / `wallet_connect.*` localized strings, `WalletConnectEventHandler*` in `Modules/Main/Workers/`.
- Risk: WC signing handlers are invoked from EVM/Stellar send flows. Removal requires unwrapping `if walletConnect...` branches in `SendTransactionService` and confirming no transaction signing path depends on WC infrastructure.

### D12 — Remove subscriptions / premium
- Android refs: `a636a96a`, `a0664ff7`.
- iOS impact: `Modules/Premium/*`, `Modules/Subscription/*`, `Core/Managers/SubscriptionManager.swift`, `Core/Managers/PurchaseManager.swift`, premium cell components, `purchase.*` / `premium.*` / `subscription.*` localized strings, StoreKit dependency in `project.pbxproj`. Unwrap 20+ premium-gating call sites (search `purchaseManager.hasActivePurchase`, `premiumEnabled`).
- Risk: secure-send / swap-protection branches in `SecuritySettingsViewModel`, `WalletViewModel`, send flows.

### B5 (partial) — hasMarketData navigation guards
- The extension exists; wrapping tap handlers in balance/transaction cells is still pending.

### C7 (partial) — Accent color asset + theme selector hide
- ThemeManager is locked; the asset catalog color swap and AppearanceView UI removal are still pending.
...
- **Unit tests:** Not run. Recommended additions in follow-up: `BlockchainType.hasMarketData` returns expected values for all supported chains; `TermsConfiguration.current.version == 5`; `AppStateManager.swapEnabled == false` after init.
- **Integration testing:** Required on macOS — verify (a) cold launch shows splash for ~2s then onboarding, (b) terms screen lists the 5 canonical items, (c) swap tab is absent in main tab bar, (d) theme is dark regardless of system appearance, (e) Quantum Chain transactions confirm chain id 20803 in signing, (f) QRC20 transfers are correctly categorized by spam manager.
...
```
docs/android-ios-port/gap-report.md                                    (new)
docs/android-ios-port/component-map.md                                 (new)
docs/android-ios-port/final-implementation-report.md                   (new)
...
1. Settings simplification (C10) — small, isolated UI removals.
2. hasMarketData nav guards + theme selector hide + accent color asset (B5/C7 follow-ups) — small, scoped.
3. WalletConnect removal (D11) — large but mechanical; do with build verification.
4. Subscription removal (D12) — large; needs careful audit of `purchaseManager.hasActivePurchase` call sites.
5. Price source router (B4) — net-new; build in isolation, unit-test the merge logic first.
