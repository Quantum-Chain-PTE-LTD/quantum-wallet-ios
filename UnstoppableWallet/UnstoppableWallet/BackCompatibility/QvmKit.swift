...
public extension Kit {
    ...
    func rawTransaction(address: QvmKit.Address, value: BigUInt, transactionInput: Data = Data(), gasPrice: GasPrice, gasLimit: Int, nonce: Int? = nil) -> Single<RawTransaction> {
    ...
    func sendSingle(rawTransaction: RawTransaction, signature: Signature, publicKey: Data) -> Single<FullTransaction> {
        Single<FullTransaction>.create { [weak self] observer in
            guard let strongSelf = self else {
                observer(.error(DisposedError()))
                return Disposables.create()
            ...
            let task = Task {
                do {
                    let result = try await strongSelf.send(rawTransaction: rawTransaction, signature: signature, publicKey: publicKey)
                    observer(.success(result))
                } catch {
                    observer(.error(error))
                }
    ...
    func getStorageAt(contractAddress: QvmKit.Address, positionData: Data, defaultBlockParameter: DefaultBlockParameter = .latest) -> Single<Data> {
    ...
    func call(contractAddress: QvmKit.Address, data: Data, defaultBlockParameter: DefaultBlockParameter = .latest) -> Single<Data> {
        Single<Data>.create { [weak self] observer in
            guard let strongSelf = self else {
                observer(.error(DisposedError()))
                return Disposables.create()
    ...
    func estimateGas(to: QvmKit.Address?, amount: BigUInt, gasPrice: GasPrice) -> Single<Int> {
        Single<Int>.create { [weak self] observer in
            guard let strongSelf = self else {
                observer(.error(DisposedError()))
                return Disposables.create()
    ...
    func estimateGas(to: QvmKit.Address?, amount: BigUInt?, gasPrice: GasPrice, data: Data?) -> Single<Int> {
        Single<Int>.create { [weak self] observer in
            guard let strongSelf = self else {
                observer(.error(DisposedError()))
                return Disposables.create()
    ...
    static func callSingle(networkManager: NetworkManager, rpcSource: RpcSource, contractAddress: QvmKit.Address, data: Data, defaultBlockParameter: DefaultBlockParameter = .latest) -> Single<Data> {
...
extension L1FeeProvider {
    ...
    public func getL1Fee(gasPrice: GasPrice, gasLimit: Int, to: QvmKit.Address, value: BigUInt, data: Data) -> Single<BigUInt> {
...
extension QIP1155Provider {
    struct DisposedError: Error {}

    public func getBalanceOf(contractAddress: QvmKit.Address, tokenId: BigUInt, address: QvmKit.Address) -> Single<BigUInt> {
        Single<BigUInt>.create { [weak self] observer in
            guard let strongSelf = self else {
                observer(.error(DisposedError()))
                return Disposables.create()
...
public extension QIP1559GasPriceProvider {
    internal struct DisposedError: Error {}
...
extension QNSProvider {
    struct DisposedError: Error {}

    public func address(domain: String) -> Single<QvmKit.Address> {
        Single<QvmKit.Address>.create { [weak self] observer in
            guard let strongSelf = self else {
                observer(.error(DisposedError()))
                return Disposables.create()
            }
