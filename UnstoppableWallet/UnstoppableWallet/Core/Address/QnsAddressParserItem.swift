...
class QnsAddressParserItem {
    static let registrars = ["qc", "qntm", "q"]

    private let provider: QNSProvider
    private let rawAddressParserItem: IAddressParserItem

    init?(rpcSource: RpcSource, rawAddressParserItem: IAddressParserItem) {
        guard let provider = try? QNSProvider.instance(rpcSource: rpcSource) else {
            return nil
        }
...
extension QnsAddressParserItem: IAddressParserItem {
