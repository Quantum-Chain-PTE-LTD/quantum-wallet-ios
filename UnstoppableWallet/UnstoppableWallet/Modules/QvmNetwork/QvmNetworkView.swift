...
struct QvmNetworkView: View {
    @StateObject private var viewModel: QvmNetworkViewModel
    @Binding private var isPresented: Bool

    init(blockchain: Blockchain, isPresented: Binding<Bool>) {
        _viewModel = .init(wrappedValue: QvmNetworkViewModel(blockchain: blockchain))
                                ...
                                Cell(
                                    ...
                                    action: {
                                        Coordinator.shared.present { _ in
                                            AddQvmSyncSourceSheetView(blockchainType: viewModel.blockchain.type)
                                                .ignoresSafeArea()
                                        }
                                    }
                                )
    ...
    @ViewBuilder func cell(source: QvmSyncSource) -> some View {
        Cell(
            middle: {
